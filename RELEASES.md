...
| **What it does** | Runs AI coding agent sessions (Claude Code, Codex, Cursor, Gemini, OpenCode) and connects them to the NEST Hub via Socket.IO and REST. |
| **Features** | Session lifecycle, auth, runner daemon, RPC handlers (git, files, terminal, uploads, slash commands, skills), multi-agent modes. |

**Releases:** [npm](https://www.npmjs.com/package/@factadev/cli) · [NEST repo](https://github.com/ctx0/nest)

---

### NEST Hub (TypeScript/Bun)

| Item | Description |
|------|-------------|
| **Repository** | [ctx0/nest](https://github.com/ctx0/nest) — `hub/` |
...
**Deployment:** Run with `nest hub` or deploy via Docker using this repo (ctx0_nest_terminal).
...
**Served by:** NEST Hub at the same origin (or static host + CORS).
...
| **What it does** | Lets users run the NEST Hub in Docker with one command; includes branding (logo, powered by facta.dev) from `public/`. |
| **Deliverables** | `Dockerfile`, `docker-compose.yml`, `.env.example`, `QUICKSTART.md`, `docs/INSTALL-FRAMEWORKS.md`, `RELEASES.md`, `ROADMAP.md`. |
| **Image** | Builds Hub + web from NEST source and overlays `public/` assets. |

**Usage:** `docker compose up -d` → Hub at http://localhost:3006.
...
| **Status** | Phase 1 — REST API and storage; realtime and full feature parity with the TypeScript Hub are pending. |
...
- **Hub / Web / Rust server:** Follow the NEST repo’s release tags and changelog.
...
- **NEST (Hub, Web, CLI source):** [GitHub ctx0/nest releases](https://github.com/ctx0/nest/releases)
